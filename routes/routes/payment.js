...
router.post('/create', authMiddleware, async (req, res) => {
  try {
    ...
    res.json({ paymentId: payment.payment_id, payAddress: payment.pay_address, payAmount: payment.pay_amount, payCurrency: payment.pay_currency, status: payment.payment_status });
  } catch (err) {
...
router.get('/status/:paymentId', authMiddleware, async (req, res) => {
  try {
    const response = await axios.get(`${NOWPAYMENTS_API}/payment/${req.params.paymentId}`, { headers: { 'x-api-key': NOWPAYMENTS_KEY } });
...
router.post('/webhook', async (req, res) => {
  try {
    ...
    if (payment_status === 'finished' || payment_status === 'confirmed') {
      const txResult = await pool.query('SELECT * FROM transactions WHERE payment_id=$1 AND status=$2', [payment_id, 'pending']);
      if (txResult.rows.length > 0) {
        const tx = txResult.rows[0];
        await pool.query('UPDATE transactions SET status=$1 WHERE payment_id=$2', ['completed', payment_id]);
...
router.get('/currencies', async (req, res) => {
  try {
    const response = await axios.get(`${NOWPAYMENTS_API}/currencies`, { headers: { 'x-api-key': NOWPAYMENTS_KEY } });
    res.json(response.data);
  } catch (err) {
