...
router.get('/users', authMiddleware, adminCheck, async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT id, email, username, balance, created_at FROM users ORDER BY created_at DESC'
    );
...
router.get('/stats', authMiddleware, adminCheck, async (req, res) => {
  try {
    ...
    const revenue = await pool.query(
      "SELECT SUM(amount) FROM transactions WHERE type='deposit' AND status='completed'"
    );
